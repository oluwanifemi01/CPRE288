...
/**
 * @brief Configure the BNO055 for heading/orientation output.
 *
 * Leaves the sensor in the requested operating mode.
 *
 * @param mode Fusion mode to use, typically IMU for relative heading or NDOF
 *        for absolute heading.
 * @return True if the BNO055 responds and was configured.
 */
...
/**
 * @brief Gets current Euler heading/roll/pitch in degrees.
 *
 * IMU must be in a fusion mode. Heading is 0-360 degrees.
 *
 * @return True if values were read, else false.
 */
bool imu_getEulerDeg(float *heading, float *roll, float *pitch);

/**
 * @brief Gets current Euler heading in degrees.
 *
 * IMU must be in a fusion mode. Heading is 0-360 degrees.
 *
 * @return True if heading was read, else false.
 */
...
/**
 * @brief Gets current calibration status.
 */
...
/**
 * @brief Gets the current IMU mode.
 */
